...
class RdbParser extends Parser {
  ...
  private def readMagicString(fileIter: Iterator[Int]): Unit = {
    ...
    val bytes =
      (1 to 5).map(_ => if (fileIter.hasNext) fileIter.next().toByte else 0.toByte).toArray
  ...
  private def readVersion(fileIter: Iterator[Int]): Unit = {
    ...
    val bytes =
      (1 to 4).map(_ => if (fileIter.hasNext) fileIter.next().toByte else 0.toByte).toArray
  ...
  private def readEncodedValue(fileIter: Iterator[Int], lengthEncoding: Int): Array[Byte] = {
    ...
    lengthEncoding match {
      ...
      case REDIS_RDB_ENC_INT16 =>
        (1 to 2).map(_ => if (fileIter.hasNext) fileIter.next().toByte else 0.toByte).toArray
  ...
  private def readMetadata(fileIter: Iterator[Int]): Unit = {
    ...
    val attributeBytes = (1 to attributeLength)
      .map(_ => if (fileIter.hasNext) fileIter.next().toByte else 0.toByte)
      .toArray
  ...
  private def readEightByteLittleEndian(fileIter: Iterator[Int]): Long = {
    ...
    val bytes =
      (1 to 8).map(_ => if (fileIter.hasNext) fileIter.next().toByte else 0.toByte).toArray
